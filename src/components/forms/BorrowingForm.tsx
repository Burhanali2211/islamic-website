...
const borrowingSchema = z.object({
  user_id: z.string().min(1, 'يجب اختيار المستعير - Borrower is required'),
  book_id: z.string().min(1, 'يجب اختيار الكتاب - Book is required'),
  due_date: z.string().min(1, 'تاريخ الإرجاع مطلوب - Due date is required'),
...
export function BorrowingForm({ onClose, record }: BorrowingFormProps) {
  ...
  const [userBorrowingSummary, setUserBorrowingSummary] = useState<any>(null);
    ...
    if (watchedUserId) {
      ...
      if (user) {
        setUserBorrowingSummary({
          ...
          canBorrow: (user.current_borrowed_count || 0) < (user.max_borrow_limit || 3)
        });
      }
    }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      {/* User Selection */}
      <div>
        <label className="block text-sm font-medium mb-2">المستعير - Borrower *</label>
        <select 
          {...register('user_id')} 
          className="w-full glass-input p-3 rounded-lg"
          disabled={!!record}
        >
          <option value="">اختر المستعير - Select Borrower</option>
        ...
        {userBorrowingSummary && (
          <div className="mt-2 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
            <div className="text-sm">
              <p>الاستعارات الحالية: {userBorrowingSummary.currentBorrowed} / {userBorrowingSummary.maxLimit}</p>
              <p className={`${userBorrowingSummary.canBorrow ? 'text-green-600' : 'text-red-600'}`}>
                {userBorrowingSummary.canBorrow ? 'يمكن الاستعارة - Can Borrow' : 'لا يمكن الاستعارة - Cannot Borrow'}
              </p>
            </div>
          </div>
        )}
      ...
      {/* Book Selection */}
      <div>
        <label className="block text-sm font-medium mb-2">الكتاب - Book *</label>
        <select 
          {...register('book_id')} 
          className="w-full glass-input p-3 rounded-lg"
          disabled={!!record}
        >
          <option value="">اختر الكتاب - Select Book</option>
          {availableBooks.map(book => (
            <option key={book.id} value={book.id}>
              {book.title} - {book.author} 
              ({book.physical_copies} نسخة متاحة)
            </option>
          ))}
        </select>
      ...
      {/* Due Date */}
      <div>
        <label className="block text-sm font-medium mb-2">تاريخ الإرجاع المتوقع - Expected Return Date *</label>
        <input 
          type="date" 
          {...register('due_date')} 
          className="w-full glass-input p-3 rounded-lg"
        ...
        <p className="text-xs text-gray-500 mt-1">
          الفترة الافتراضية للاستعارة 14 يوماً - Default borrowing period is 14 days
        </p>
      </div>

      {/* Notes */}
      <div>
        <label className="block text-sm font-medium mb-2">ملاحظات - Notes</label>
        <textarea 
          {...register('notes')} 
          className="w-full glass-input p-3 rounded-lg" 
          rows={3}
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
        <h4 className="font-medium text-blue-900 dark:text-blue-100 mb-2">
          قواعد الاستعارة - Borrowing Rules
        </h4>
        <ul className="text-sm text-blue-800 dark:text-blue-200 space-y-1">
          <li>• الحد الأقصى للاستعارة: 3 كتب للطلاب، 10 كتب للأساتذة</li>
          <li>• فترة الاستعارة الافتراضية: 14 يوماً</li>
          <li>• يمكن التجديد مرتين كحد أقصى</li>
          <li>• غرامة التأخير: 1 ريال لكل يوم</li>
          <li>• Maximum borrowing: 3 books for students, 10 for teachers</li>
          <li>• Default period: 14 days</li>
      ...
      {userBorrowingSummary && !userBorrowingSummary.canBorrow && (
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
          <p className="text-red-800 dark:text-red-200 text-sm">
            ⚠️ هذا المستخدم وصل للحد الأقصى من الاستعارات
            <br />
            This user has reached the maximum borrowing limit
          </p>
        </div>
      )}
      ...
      <div className="flex justify-end space-x-3 pt-6 border-t border-gray-200 dark:border-gray-700">
        ...
        >
          {isSubmitting ? 'جاري الحفظ...' : 'تأكيد الاستعارة - Confirm Borrowing'}
        </button>
      </div>
    </form>
  );
